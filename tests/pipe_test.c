...
{	static short buffer [PIPE_TEST_LEN] ;
	...
	*/
	if ((outfile = sf_open_fd (pipefd [1], SFM_WRITE, &sfinfo_write, SF_TRUE)) == NULL)
	{	printf ("\n\n\tLine %d : unable to create unseekable pipe for write type \"%s\".\n", __LINE__, ext) ;
		printf ("\t%s\n\n", sf_strerror (outfile)) ;
		exit (1) ;
		} ;

	if (sf_error (outfile) != SF_ERR_NO_ERROR)
	{	printf ("\n\n	Line %d : unable to open unseekable pipe for write type \"%s\".\n\n", __LINE__, ext) ;
		exit (1) ;
		} ;
	...
	*/
	if ((infile_piped = sf_open_fd (pipefd [0], SFM_READ, &sfinfo_read, SF_TRUE)) == NULL)
	{	printf ("\n\n\tLine %d : unable to create unseekable pipe for read type. \"%s\".\n\n", __LINE__, ext) ;
		exit (1) ;
		} ;

	if (sf_error (infile_piped) != SF_ERR_NO_ERROR)
	{	printf ("\n\n\tLine %d : unable to open unseekable pipe for read type. \"%s\".\n\n", __LINE__, ext) ;
		exit (1) ;
		} ;
	...
	*/
	...
	if (memcmp (buffer, data, PIPE_TEST_LEN) != 0)
	{	printf ("\n\n\tLine %d : unseekable pipe test failed for file type. \"%s\".\n\n", __LINE__, ext) ;
		retval = 1 ;
		} ;
