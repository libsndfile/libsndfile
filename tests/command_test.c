...
static	void	cue_label_bounds_test	(const char *filename) ;
...
{	int		do_all = 0 ;
	...
	{	/* 2500 is close to the largest number of cues possible because of block sizes (enforced in aiff.c, wav.c) */
		...
		cue_label_bounds_test ("cue_label.wav") ;
...
static void
cue_label_bounds_test (const char *filename)
{	/* A WAV file with one cue point and a LIST/adtl 'labl' whose text is
	** 260 bytes. The label is longer than SF_CUE_POINT.name (256 bytes), so
	** the reader must still leave the stored name NUL terminated.
	*/
	static const unsigned char wav_with_long_cue_label [] =
	{	0x52, 0x49, 0x46, 0x46, 0x68, 0x01, 0x00, 0x00, 0x57, 0x41, 0x56, 0x45,
		...
		0x4c, 0x49, 0x53, 0x54, 0x14, 0x01, 0x00, 0x00, 0x61, 0x64, 0x74, 0x6c,
		0x6c, 0x61, 0x62, 0x6c, 0x08, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
	...
	print_test_name ("cue_label_bounds_test", filename) ;

	dump_data_to_file (filename, wav_with_long_cue_label, sizeof (wav_with_long_cue_label)) ;
	...
	exit_if_true (
		sf_command (file, SFC_GET_CUE, &cues, sizeof (cues)) != SF_TRUE,
		"\n\nLine %d : SFC_GET_CUE failed.\n\n", __LINE__
		) ;

	exit_if_true (
		memchr (cues.cue_points [0].name, 0, sizeof (cues.cue_points [0].name)) == NULL,
		"\n\nLine %d : cue label is not NUL terminated.\n\n", __LINE__
		) ;

	sf_close (file) ;
	unlink (filename) ;
	puts ("ok") ;
} /* cue_label_bounds_test */
