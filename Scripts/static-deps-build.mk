...
Build/Stamp/configure : Build/Stamp/install-libs configure
	...
	sed -i 's#^EXTERNAL_CODEC_CFLAGS.*#EXTERNAL_CODEC_CFLAGS = -I$(pwd)/Build/include#' src/Makefile
	sed -i 's#^EXTERNAL_CODEC_LIBS.*#EXTERNAL_CODEC_LIBS = -static $(pwd)/Build/lib/libFLAC.la $(pwd)/Build/lib/libogg.la $(pwd)/Build/lib/libvorbis.la $(pwd)/Build/lib/libvorbisenc.la -dynamic #' src/Makefile
	make clean
	touch $@
