...
/*
** 32-bit PCM support was added to the FLAC reference library in FLAC 1.4.0,
** which is the first release with FLAC_API_VERSION_CURRENT >= 12. When built
** against an older FLAC, 32-bit calls are rejected at the function level.
*/
#if defined (FLAC_API_VERSION_CURRENT) && (FLAC_API_VERSION_CURRENT >= 12)
#define HAVE_FLAC_32BIT 1
#else
#define HAVE_FLAC_32BIT 0
#endif
