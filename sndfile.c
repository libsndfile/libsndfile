...
ErrorStruct SndfileErrors [] =
{
  ...
  { SFE_BW64_PEAK_B4_FMT    , "Error in BW64 file. 'PEAK' chunk found before 'fmt ' chunk." },
  { SFE_BW64_NO_DATA        , "Error in BW64 file. No 'data' chunk marker." },
...
{	SF_PRIVATE 	*psf ;
	...
	printf("sf_open: going to psf_open_file\n");
...
{	SF_PRIVATE *psf = (SF_PRIVATE *) sndfile ;
	...
	{	case SFC_SET_NORM_FLOAT :
		...
		case SFC_SET_AXML_INFO :
			{	int format = SF_CONTAINER (psf->sf.format) ;
			/*Only WAV and BW64 supports the support the AXML chunk. */
			if (format != SF_FORMAT_WAV && format != SF_FORMAT_WAVEX && format != SF_FORMAT_BW64)
				return SF_FALSE ;
	  ...
	  case SFC_SET_CHNA_INFO :
				{	int format = SF_CONTAINER (psf->sf.format) ;
					/*Only WAV and BW64 supports the support the CHNA chunk. */
						if (format != SF_FORMAT_WAV && format != SF_FORMAT_WAVEX && format != SF_FORMAT_BW64)
							return SF_FALSE ;
...
{	int		error, format ;
  ...
  printf("SF_CONTAINER: %d\n", SF_CONTAINER (psf->sf.format));
